...
function [d,J] = get_mECENV(t, mECENV, pars, ct, forcingVariables, tForcing)
global infosgr2 
%% Forcing variables 

tForcing = double(forcingVariables.dt(:));                % always column
N_forcing = forcingVariables.N(:);     % always column
irradiance_forcing=  forcingVariables.irradiance(:);
P_forcing= forcingVariables.P(:);
temp_forcing = forcingVariables.temp(:);

if forcingVariables.photoPeriod == 1
    I_forcing = get_irradiance(t, irradiance_forcing); %obtaining the irradiance in mol gamma m-2 h-1 at time t
else
    I_forcing = irradiance_forcing; 
end


CO2  = forcingVariables.CO2; 

N = interp1(tForcing,N_forcing,t); 
I = interp1(tForcing,I_forcing,t); 
P = interp1(tForcing,P_forcing,t); 
T = interp1(tForcing, temp_forcing, t);
ct_value = interp1(tForcing,ct,t); 
...
j_CO2m_CT = pars.j_CO2m * ct_value; % mol CO2 mol V-1 h-1, max volume specific CO2 uptake rate
k_I_CT = pars.k_I * ct_value; % mol gamma mol PSU-1 h-1, dissociationrate of photosynthetic products
% k_I_CT = k_I * 1; % mol gamma mol PSU-1 h-1, dissociationrate of photosynthetic products

j_ECAm_CT = pars.j_ECAm * ct_value; %mol C mol V-1 h-1, max volume specific carbon assimilation

j_ENAm_CT = pars.j_ENAm * ct_value; %mol N mol V-1 h-1, max volume specific nitrogen assimilation

k_EC_CT = pars.k_EC * ct_value; %h-1, C reserve turnover
k_EN_CT = pars.k_EN * ct_value; %h-1, N reserve turnover

j_ECM_CT = pars.j_ECM * ct_value; %mol C mol V-1 h-1, volume specific maitenance rate paid by C reserve
j_ENM_CT = pars.j_ENM * ct_value; %mol N mol V-1 h-1, volume specific maitenance rate paid by N reserve
...
% Carbon into reserve
% Specific CO2 flux
j_CO2 = j_CO2m_CT * (CO2 / (CO2 + pars.K_C)); % mol CO2 mol V-1 h-1
...
j_EN_A = j_ENAm_CT * (N / (N + pars.K_N)); % mol N mol V-1 h-1
...
% % Stocking flux 
[J] = [ct_value; j_CO2; j_I; j_EC_A; j_EN_A; j_EC_C; j_EN_C; j_EC_MC; j_EN_MN; j_V_MC;j_V_MN; ...
    j_V_M; j_EC_G; j_EN_G; j_VG; j_EC_R ; j_EN_R; r; I ; CO2; N; P; T]';
end