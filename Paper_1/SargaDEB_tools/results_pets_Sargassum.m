...
%%
function results_pets_Sargassum(par, metaPar, txtPar, data, auxData, metaData, txtData, weights)
        ...
        if exist(['custom_results_', pets{i}, '.m'], 'file')
          ...
          if isfield(metaData.(pets{i}), 'grp') % first working on grouped graphs
            ...
            for ii = 1:n_grpSets % scan the group-sets
              ...
              counter_fig = counter_fig + 1; legend = cell(0,2);
              % subplot(2,2, counter_fig)
              ...
              for jj = 1:n_nms % scan names of data sets in group set ii 
                if length(n_nms) < maxGroupColourSize  % choosing the right set of colours depending on the number of sets to plot
                  plotColours = pastel_jet(n_nms); 
                else
                  plotColours = pastel_jet(4);
                end
                xData = st.(nms{jj})(:,1); 
                yData = st.(nms{jj})(:,2);
                xPred = st.(nms{jj})(:,1); 
                ...
                if n_nms == 1
                  if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nms{jj}) && auxData.(pets{i}).treat.(nms{jj}){1} == 0
                    zplot(xPred, yPred, 'o', 'Color',plotColours{2}, 'Markersize',3)
                  else
                  ...
                  if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nms{jj}) && auxData.(pets{i}).treat.(nms{jj}){1} == 0
                    ...
                    plot(xPred, yPred, 'Color',plotColours(jj,:), 'linewidth',2)
                  end
                  plot(xData, yData, '.', 'Color',plotColours(jj,:), 'Markersize',15)
                  if length(txtData.(pets{i}).label.(nms{jj})) == 3
                    legend = [legend; {{'.', 15, 2, plotColours(jj,:), plotColours(jj,:)}, txtData.(pets{i}).label.(nms{jj}){3}}];
                  else
                    legend = [legend; {{'.', 15, 2, plotColours(jj,:), plotColours(jj,:)}, nms{jj}}];
                  end 
                end
          ...
          for j = 1:nst % now plot non-grouped plots
            ...
            k = size(Var, 2);
            figure; 
            counter_fig = counter_fig + 1; hold on
            % subplot(2,2, counter_fig)
            nFig = [txt0(counter_fig < 10), num2str(counter_fig)]; 
            set(gca,'Fontsize',15); 
            set(gcf,'PaperPositionMode','manual');
            ...
            if k == 2 % uni-variate data set
              ...
              if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nm{j}) && auxData.(pets{i}).treat.(nm{j}){1} == 0
                plot(xPred,yPred, 'o', 'Markersize',3, 'Color', plotColours(j,:))
              else 
                try
                  plot(xPred,yPred, 'Color', plotColours(j,:), 'linewidth',2); hold on; 
                catch
                  keyboard;
                end
              end
              plot(xData,yData, '.', 'Markersize',15, 'Color', plotColours(j,:))
