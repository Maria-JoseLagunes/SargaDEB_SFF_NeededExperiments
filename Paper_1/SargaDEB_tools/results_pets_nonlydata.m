...
%%
function results_pets_newColors(par, metaPar, txtPar, data, auxData, metaData, txtData, weights)
...
% my_pet_res.html is written in the directory entries, and deleted by run_collection, which writes another file with that name in directory entries_web.
  ...
  if n_pets > 1 % MRE, SMAE, SMSE, lossf and cv are used in addCV to report in addCV.html, which is why they are added to metaPar here
    ...
    MRE = 0; SMAE = 0; SMSE = 0;  % append MRE, SMAE and SMSE to metaPar as means over aplotll pets
    for i = 1 : n_pets
        ...
        if exist(['custom_results_', pets{i}, '.m'], 'file')
          ...
          if isfield(metaData.(pets{i}), 'grp') % first working on grouped graphs
            ...
            for ii = 1:n_grpSets % scan the group-sets
              ...
              for jj = 1:n_nms % scan names of data sets in group set ii 
                ...
                if n_nms == 1
                  if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nms{jj}) && auxData.(pets{i}).treat.(nms{jj}){1} == 0
                    plot(xPred, yPred, 'o', 'Color',plotColours{2}, 'Markersize',30)
                  else
                    plot(xPred, yPred,'Color',plotColours{2}, 'linewidth',2.5)
                  end
                  plot(xData, yData, '.', 'Color',plotColours{1}, 'Markersize',30)
                else % more than 1 data set in group set ii
                  if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nms{jj}) && auxData.(pets{i}).treat.(nms{jj}){1} == 0
                    plot(xPred, yPred, 'o', 'Color',plotColours{mod(jj, maxGroupColourSize)}, 'Markersize',30)
                  else
                    plot(xPred, yPred, 'Color',plotColours{mod(jj, maxGroupColourSize)}, 'linewidth',2.5)
                  end
                  plot(xData, yData, '.', 'Color',plotColours{mod(jj, maxGroupColourSize)}, 'Markersize',30)
                  if length(txtData.(pets{i}).label.(nms{jj})) == 3
                    legend = [legend; {{'.', 22, 2, plotColours{mod(jj, maxGroupColourSize)}, plotColours{mod(jj, maxGroupColourSize)}}, txtData.(pets{i}).label.(nms{jj}){3}}];
                  else
                    legend = [legend; {{'.', 22, 2, plotColours{mod(jj, maxGroupColourSize)}, plotColours{mod(jj, maxGroupColourSize)}}, nms{jj}}];
                  end 
                end
              ...
              maxLength = 25;  % threshold for line break
              ylabel_set = ([txtData.(pets{i}).label.(nms{1}){2}, ', ', txtData.(pets{i}).units.(nms{1}){2}]);
              if strlength(ylabel_set) > maxLength
                  spaceIdx = strfind(ylabel_set, ' ');
                  breakIdx = spaceIdx(find(spaceIdx > maxLength/2, 1));  % break near middle
                  if ~isempty(breakIdx)
                      ylabel_set = {ylabel_set(1:breakIdx-1), ylabel_set(breakIdx+1:end)};
                  end
              end
              ylabel(ylabel_set);
              try
                title(['\it',metaData.(pets{i}).grp.title{ii}], 'FontSize',22, 'FontWeight','normal'); 
              catch
                title(''); 
              end
          ...
          for j = 1:nst % now plot non-grouped plots
            ...
            if k == 2 % uni-variate data set
              ...
              if isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nm{j}) && auxData.(pets{i}).treat.(nm{j}){1} == 0
                plot(xPred,yPred, 'ob', 'Markersize',1.5)
              else 
                try
                  plot(xPred,yPred, 'Color', [0.827450980392157,0.121568627450980,0.0666666666666667], 'linewidth',2.5)
                catch
                  keyboard;
                end
              end
              plot(xData,yData, '.', 'Markersize',30, 'Color', [0.956862745098039,0.478431372549020,0])
              ...
              ylabel_set = ([aux.label.(fieldsInCells{1}{end}){2}, ', ', aux.units.(fieldsInCells{1}{end}){2}]);
              maxLength = 25;  % threshold for line break
              if strlength(ylabel_set) > maxLength
                  spaceIdx = strfind(ylabel_set, ' ');
                  breakIdx = spaceIdx(find(spaceIdx > maxLength/2, 1));  % break near middle
             ...
             ylabel(ylabel_set);
              ...
              try
                title(['\it',aux.title.(fieldsInCells{1}{end})], 'FontSize',22, 'FontWeight','normal');
              catch
                title('');
              end
              if abs(results_output) >= 3 
            ...
            elseif isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat, nm{j}) && ...
              ...
              if k == 3 && length(txtData.(pets{i}).units.(nm{j})) == 3 && length(txtData.(pets{i}).label.(nm{j})) == 3 % yyaxis
                ...
                elseif treat{1} == 0 % do not interpolate 1st and 2nd independent var and plot markers
                    ...
                    yyaxis left
                    plot(xPred, yPred(:,1), 'o', 'Color','r', 'Markersize',2.5)
                    plot(xData, y1Data, '.', 'Color','r', 'Markersize',30)
                    ylabel_set = ([txtData.(pets{i}).label.(nm{j}){2}, ', ', txtData.(pets{i}).units.(nm{j}){2}]);
                    ...
                    ylabel(ylabel_set)
                    yyaxis right
                    plot(xPred, yPred(:,2), 'o', 'Color','b', 'Markersize',30)
                    plot(xData, y2Data, '.', 'Color','b', 'Markersize',30)

                    maxLength = 25;  % threshold for line break
                    ylabel_set = ([txtData.(pets{i}).label.(nm{j}){3}, ', ', txtData.(pets{i}).units.(nm{j}){3}]);
                    if strlength(ylabel_set) > maxLength
                        spaceIdx = strfind(ylabel_set, ' ');
                        breakIdx = spaceIdx(find(spaceIdx > maxLength/2, 1));  % break near middle
                        if ~isempty(breakIdx)
                  ...
                  yyaxis left
                  plot(xPred, yPred(:,1), '-', 'Color','r', 'linewidth',2.5)
                  plot(xData, y1Data, '.', 'Color','r', 'Markersize',30)
                  maxLength = 25;  % threshold for line break
                  ylabel_set = ([txtData.(pets{i}).label.(nm{j}){3}, ', ', txtData.(pets{i}).units.(nm{j}){3}]);
                  if strlength(ylabel_set) > maxLength
                  ...
                  yyaxis right
                  plot(xPred, yPred(:,2), '-', 'Color','b', 'linewidth',2.5)
                  plot(xData, y2Data, '.', 'Color','b', 'Markersize',30)
                  maxLength = 25;  % threshold for line break
                  ylabel_set = ([txtData.(pets{i}).label.(nm{j}){3}, ', ', txtData.(pets{i}).units.(nm{j}){3}]);
                  if strlength(ylabel_set) > maxLength
                ...
                if treat{1} == 0 % do not interpolate 1st and 2nd independent var and plot markers
                  ...
                  for ii = 1: n_sets2plot
                    ...
                    if n_sets2plot == 1
                      ...
                      legend = [legend; {{'.', 22, 2, plotColours{mod(ii, maxGroupColourSize)}, plotColours{mod(ii, maxGroupColourSize)}}, txt_ii}];
                    end
                  end
                  xlabel([txtData.(pets{i}).label.(nm{j}){1}, ', ', txtData.(pets{i}).units.(nm{j}){1}]);
                  maxLength = 25;  % threshold for line break
                  ...
                  try
                    title(['\it',txtData.(pets{i}).title.(nm{j})], 'FontSize',22, 'FontWeight','normal');
                  catch
                    title('')
                  end
                  plotNm = ['results_', pets{i}, '_', nFig];
                ...
                elseif treat{1} == 1 % do not interpolate 2nd independent var and plot curves
                  ...
                  for ii = 1: n_sets2plot
                    ...
                    if n_sets2plot == 1
                      ...
                      plot(xPred, yPred, 'Color', plotColours{mod(ii, maxGroupColourSize)}, 'linewidth', 2.5)
                      plot(xData, yData, '.', 'Color', plotColours{mod(ii, maxGroupColourSize)}, 'Markersize',30)
                      if isnumeric(treat{2}); txt_ii = [num2str(treat{2}(ii)), ' ', txtData.(pets{i}).units.treat.(nm{j})]; else;  txt_ii = treat{2}(ii); end
                      legend = [legend; {{'.', 22, 2, plotColours{mod(ii, maxGroupColourSize)}, plotColours{mod(ii, maxGroupColourSize)}}, txt_ii}];
                    end
                ...
                elseif treat{1} > 1 % interpolate 2nd independent var and plot mesh or surface
                  ...
                  for ii = 1:k-1  % scan y-values
                    for jj = 1:nX % scan x-values
                      ...
                      plot3(xData(jj), yData(ii), zData(jj,ii), '.', 'Color',plotColours{1}, 'Markersize',30)
                    end
                  end
                  % insert clock
                  xlabel([txtData.(pets{i}).label.(nm{j}){1}, ', ', txtData.(pets{i}).units.(nm{j}){1}]);
                  maxLength = 25;  % threshold for line break
                  ylabel_set = ylabel([txtData.(pets{i}).label.treat.(nm{j}), ', ', txtData.(pets{i}).units.treat.(nm{j})]);
                  if strlength(ylabel_set) > maxLength
                    spaceIdx = strfind(ylabel_set, ' ');
                    breakIdx = spaceIdx(find(spaceIdx > maxLength/2, 1));  % break near middle
                    if ~isempty(breakIdx)
            ...
            elseif isfield(auxData.(pets{i}), 'treat') && isfield(auxData.(pets{i}).treat,nm{j}) % tri-variate data, assuming that threat.(nm{j}){1} = 2 or 3 (interpolation in 3 vars)
              ...
              if length(treat)<2 || length(treat{2})<3
                ...
                for k = 1:n_t % scan time points
                  ...
                  for ii = 1:n_y  % scan y-values
                    for jj = 1:n_x % scan x-values
                      ...
                      plot3(xData(jj), yData(ii), zData_k(jj,ii), '.', 'Color',plotColours{1}, 'Markersize',30)
                    end
                  end
                  xlim([x_min, x_max]); ylim([y_min, y_max]); zlim([z_min, z_max]);
                  xlabel([txtData.(pets{i}).label.(nm{j}){1}, ', ', txtData.(pets{i}).units.(nm{j}){1}]);
                  maxLength = 25;  % threshold for line break
                  ylabel_set =  ylabel([txtData.(pets{i}).label.(nm{j}){2}, ', ', txtData.(pets{i}).units.(nm{j}){2}]);
                  if strlength(ylabel_set) > maxLength
                      spaceIdx = strfind(ylabel_set, ' ');
                      breakIdx = spaceIdx(find(spaceIdx > maxLength/2, 1));  % break near middle
                      if ~isempty(breakIdx)
                  ...
                  if abs(results_output) >= 3
                    ...
                    set(gcf,'PaperType','A4');
                    set(gcf,'PaperPosition',[1. 1. 22. 14.]);
                    print([nmDir,'/',plotNm_pdf],'-vector','-dpdf')
                ...
                if ismac || isunix
                  ...
                  system(['apngasm64 ../',nmDir,'.pdf frame*.pdf']);
                else
...
function plotColours4AllSets = listOfPlotColours4UpTo13Sets
    ...
    plotColours4AllSets = cell(1, 12);
    for n = 2:13
    cmap = get_idf_palette(n);
    plotColours4AllSets{n - 1} = mat2cell(cmap, ones(1, n), 3);
    end
