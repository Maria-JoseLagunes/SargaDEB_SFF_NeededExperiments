...
% DEB tool M function
% Difference between sgr2_Sargassum and sgr2 from the DEB tool_M package is
% the output vector which in this case includes the jEC flux, the equations
% stay the same only modification of how it is called in sgr2.m called as
% j_EC, in here called as jEC and included in the vector output 
%
% Last modified : 03/2025 by Maria-José Lagunes
%%
function [r,jEC, jEM, jVM, jER, info] = ...
      sgr2_Sargassum (m_E, k_E, j_EM, y_EV, j_VM, a, r0)
  ...
  jEC = [0;0]; jEM = [0;0]; jVM = [0;0]; % initiate vars
  
  while (f^2 > 1e-15) & (i < n) % test norm
    for j = 1:2 % obtain jEM jVM jER given r
      jEC(j) = m_E(j) * (k_E(j) - r); % catabolic rate
      if a(j) == 0 % switch model
        % jEM(j) = max(1e-6, min(j_EM(j), jEC(j)));
        jEM(j) = min(j_EM(j), jEC(j)); 
        % jVM(j) = j_VM(j) * (1 - jEM(j)/ j_EM(j));
        jVM(j) = (j_EM(j) - jEM(j))* (y_EV(j))^(-1); 
      else % preference model
        C = - jEC(j) * (jEC(j) + y_EV(j) * j_VM(j));
        B = C + ((1 - a(j)) * jEC(j) + y_EV(j) * j_VM(j)) * j_EM(j);
        A = a(j) * j_EM(j)^2;
        jVM(j) = j_VM(j) * 2 * A/ (2 * A + sqrt(B^2 - 4 * A * C) - B);
        jEM(j) = j_EM(j) * (1 - jVM(j)/ j_VM(j));
      end
    end
    R = max(1e-6, (jEC - jEM) ./ y_EV); % compounding rates
