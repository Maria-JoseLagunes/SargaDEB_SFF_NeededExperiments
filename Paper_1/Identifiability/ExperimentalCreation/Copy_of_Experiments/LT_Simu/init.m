...
for j = 1:length(timeFields)
    ...
    if contains(fieldName, dataset)
        ...
        tFinal = [tFinal; (auxData.(pets{1}).time.(fieldName) * 24) + 1]; %a verifier sile +1 ne cause pas de problèmes 
    end
end
...
elseif contains(dataset, 'nutrientSimulations')
       ...
       t0 = 1; % h, initial time
       tFinal_days = 1500; %d, time of experience
       tFinal = ((tFinal_days + 1)  * 24); % hours
      ...
      colorMatrix = colormap(pastel_jet(length(maxVal)));
      forcingConditions.photoPeriod = 0; 
